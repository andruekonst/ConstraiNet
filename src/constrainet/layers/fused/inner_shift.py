...
class InnerShiftFusedConstrainNetwork(torch.nn.Module):
    ...
    def forward(self, z):
        ...
        inside = (~common_ind).unsqueeze(1) * z
        al = common_alpha.unsqueeze(1)
        gamma = al
        p = self.pivot
        outside = common_ind.unsqueeze(1) * ((gamma + (1 - gamma) * al) * p + (1 - gamma) * (1 - al) * z)
        return inside + outside


class InnerShiftFusedNNOptSolver(NNOptSolver):
    def make_projection_nn(self, problem: LinearConstraintsOptProblem):
        ...
        projection_nn = make_fused_system_nn(
            problem.constraints,
            InnerShiftFusedConstrainNetwork,
            rng=rng,
            bbox=bbox,
