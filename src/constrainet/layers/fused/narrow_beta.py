...
class NarrowBetaStepToInteriorFusedConstrainFn(torch.autograd.Function):
    @staticmethod
    def forward(ctx, z, At, b, pivot):
        ...
        ind = (sign_arg >= 0)
        # ind shape: (n_sample, n_constraints)
        ...
        ctx.save_for_backward(At, ind, common_ind, alpha, ray)
        return z + common_ind.unsqueeze(1) * (common_alpha.unsqueeze(1) * ray)

    @staticmethod
    def backward(ctx, grad_output):
        At, ind, common_ind, alpha, ray = ctx.saved_tensors
        ...
        # intersection_beta = ...
        # beta = torch.min(beta, intersection_beta)
        ...
        raise NotImplementedError("Need to intersect with A and avoid stepping out of constraints")
        return grad_z, None, None, None


class NarrowBetaStepToInteriorFusedConstrainNetwork(torch.nn.Module):
    ...
    def forward(self, z):
        return NarrowBetaStepToInteriorFusedConstrainFn.apply(z, self.At, self.b, self.pivot)


class NarrowBetaStepToInteriorFusedNNOptSolver(NNOptSolver):
    def make_projection_nn(self, problem: LinearConstraintsOptProblem):
        ...
        projection_nn = make_fused_system_nn(
            problem.constraints,
            NarrowBetaStepToInteriorFusedConstrainNetwork,
            rng=rng,
            bbox=bbox,
