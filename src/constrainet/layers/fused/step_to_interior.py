...
class StepToInteriorFusedConstrainFn(torch.autograd.Function):
    @staticmethod
    def forward(ctx, z, At, b, pivot):
        ...
        ctx.save_for_backward(At, ind, common_ind, alpha)
        return z + common_ind.unsqueeze(1) * (common_alpha.unsqueeze(1) * ray)

    @staticmethod
    def backward(ctx, grad_output):
        At, ind, common_ind, alpha = ctx.saved_tensors
...
class StepToInteriorFusedConstrainNetwork(torch.nn.Module):
    ...
    def forward(self, z):
        return StepToInteriorFusedConstrainFn.apply(z, self.At, self.b, self.pivot)


class StepToInteriorFusedNNOptSolver(NNOptSolver):
    def make_projection_nn(self, problem: LinearConstraintsOptProblem):
        ...
        projection_nn = make_fused_system_nn(
            problem.constraints,
            StepToInteriorFusedConstrainNetwork,
            rng=rng,
            bbox=bbox,
